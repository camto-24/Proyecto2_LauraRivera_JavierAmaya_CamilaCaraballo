...
dane_mpios <- read_excel("data/Listado_Departamentos.xlsx") %>%
  ...
  mutate(
    # Es vital asegurar que los códigos sean texto para que el join funcione bien
    cod_depto = as.character(cod_depto),
    cod_mpio  = as.character(cod_mpio)
  )

# =============================================================================
# 3. Preparación general: IDs y llaves
# =============================================================================
df_modelo <- df_bivariado %>%
  mutate(
    # --- IMPORTANTE: Convertir identificadores de negocio a TEXTO puro ---
    # Esto asegura que salgan como "01", "101", etc., y no como números sumables
    directorio_char  = as.character(directorio),
    secuencia_p_char = as.character(secuencia_p),
    orden_char       = as.character(orden),

    # Creación de llaves técnicas (Concatenadas para el sistema)
    id_hogar   = paste0(directorio, "-", secuencia_p),
    id_persona = paste0(directorio, "-", secuencia_p, "-", orden),
    
    # Asegurar tipos de dato para cruce geográfico
    cod_mpio   = as.character(cod_mpio),
    cod_depto  = as.character(cod_depto),
    
    # Llave compuesta para geografía (Depto + Mpio)
    id_geografia = paste0(cod_depto, cod_mpio)
  )

# =============================================================================
# 4. DIMENSIONES (Corrección Observación 3: Normalización y separación)
# =============================================================================
...
# ---- Dimensión Hogar ---------------------------------------------------------
# Observación 1: Mantenemos el hogar como dimensión.
# AJUSTE: Ponemos Directorio y Secuencia PRIMERO para fácil lectura
dim_hogar <- df_modelo %>%
  select(
    directorio = directorio_char,   # Renombramos para exportar limpio
...
dim_parentesco <- df_modelo %>%
  ...
  distinct() %>%
  arrange(parentesco) %>%
  mutate(id_parentesco = row_number()) %>%
  relocate(id_parentesco)
...
dim_educacion <- df_modelo %>%
  ...
  mutate(id_educacion = row_number()) %>%
  relocate(id_educacion)

# ---- Dimensión Estado de Salud (Percepción) ----------------------------------
dim_estado_salud <- df_modelo %>%
  select(estado_salud) %>%
  distinct() %>%
  arrange(estado_salud) %>%
  mutate(id_estado_salud = row_number()) %>%
  relocate(id_estado_salud)

# ---- Dimensión Enfermedad Crónica (Diagnóstico) ------------------------------
dim_cronica <- df_modelo %>%
  select(cronica_12m) %>%
  distinct() %>%
  arrange(cronica_12m) %>%
  mutate(id_cronica = row_number()) %>%
  relocate(id_cronica)

# ---- Dimensión Frecuencia de Paquetes ----------------------------------------
dim_paquetes <- df_modelo %>%
  select(frecuencia_paquetes) %>%
  distinct() %>%
  mutate(id_paquetes = row_number()) %>%
  relocate(id_paquetes)

# ---- Dimensión Consumo (Bebidas azucaradas) ----------------------------------
...
fact_personas <- df_modelo %>%
  # Joins para obtener los IDs numéricos (Surrogate Keys)
  left_join(dim_sexo,         by = "sexo") %>%
  left_join(dim_etario,       by = "grupo_edad") %>%
  left_join(dim_parentesco,   by = "parentesco") %>%
  left_join(dim_educacion,    by = "nivel_educativo") %>%
  left_join(dim_estado_salud, by = "estado_salud") %>%
  left_join(dim_cronica,      by = "cronica_12m") %>%
  left_join(dim_paquetes,     by = "frecuencia_paquetes") %>%
  ...
  select(
    # --- IDENTIFICACIÓN CLARA (Tus Observaciones) ---
    # Estas son las columnas originales para que sepas quién es "Hogar 1, Persona 1"
    directorio  = directorio_char,
    secuencia_p = secuencia_p_char,
    orden       = orden_char,

    # --- Llaves Técnicas (necesarias para relacionar en BI, pero secundarias visualmente) ---
    id_persona,
    id_hogar,
    
    # --- IDs de las Variables (Dimensiones) ---
    # Nota: Estos números conectan con las tablas Dimensión.
    # Por ejemplo, si id_sexo es 1, vas a Dim_Sexo y ves que 1 = Hombre.
    id_sexo,
    id_etario,
    id_parentesco,
    id_educacion,
