...
df_salud_ampliado <- salud %>%
  select(DIRECTORIO, SECUENCIA_P, ORDEN,
         ...
         P6127,       # Estado de salud general
         P3003S1     # Frecuencia de consumo alimentos de paquete
     )
...
# 1. SALUD
n_dupli_salud_ampliado <- df_salud_ampliado %>% 
  group_by(DIRECTORIO, SECUENCIA_P, ORDEN) %>% 
  filter(n() > 1) %>% 
  nrow()
print(paste("Duplicados en Salud:", n_dupli_salud_ampliado))
...
df_final <- df_final %>%
  rename(
    ...
    # --- Variables de Control (Sociodemográficas y confusoras) ---
    #P6127: Estado general de salud
    estado_salud = P6127,
    #P3003S1: Frecuencia de consumo alimentos de paquete 
    frecuencia_paquetes = P3003S1,
    # P6020: Sexo
    sexo = P6020,
    # P6040: Edad
    edad = P6040,
...
VARIABLES NUMÉRICAS: Las variables que son verdaderamente numéricas teniendo en cuenta las definiciones de la ECV-2023 son:

-   Edad

-   Ingreso per capita

-   Estado salud (variable nueva agregada)

-   Frecuencia paquete (variable nueva agregada)

```{r}
summary(df_final$edad)
summary(df_final$ingreso_pc)
summary(df_final$fex)
summary(df_final$estado_salud)
summary(df_final$frecuencia_paquetes)
...
hist(log(df_final$frecuencia_paquetes))
hist(log(df_final$estado_salud))
class(df_final$edad)
class(df_final$ingreso_pc)
class(df_final$fex)
class(df_final$frecuencia_paquetes)
class(df_final$estado_salud)
```

VARIABLES CATEGÓRICAS: - Directorio - Secuencia_p - orden - Sexo - Parentesco - Nivel educativo más alto alcanzado - ordinal - Diagnóstico de enfermedad crónica - Consumo de bebidas azucaradas - Frecuencia de consumo de bebidas azucaradas - Estado general de salud - Frecuencia de consumo paquetes - ordinales - Departamento - Municipio.
...
df_final <- df_final %>%
  mutate(
    ...
    frecuencia_paquetes = factor(frecuencia_paquetes),
    estado_salud        = factor(estado_salud)
  )
...
df_final <- df_final %>%
  ...
  # 1. Reemplazar NA reales por categoría 7
    frecuencia_paquetes = ifelse(is.na(frecuencia_paquetes), 7, frecuencia_paquetes),

    # 2. Convertir a factor con etiquetas
    frecuencia_paquetes = factor(frecuencia_paquetes,
                                 levels = 1:7,
                                 labels = c("Todos los días (≥2 veces/día)",
                                            "Todos los días (1 vez/día)",
...
No existen por construcción de las preguntas y documentación de la ECV-DANE-2023. Sin embargo, se construye un valor artificial de No Responde/NA para los datos faltantes en la variable de *frecuencia_paquetes* con el objetivo de capturar el sesgo de información.
...
```{r}
# Se revisa si las variables nuevas agregadas tienen NA
sum(is.na(df_final$estado_salud)) 
sum(is.na(df_final$frecuencia_paquetes)) 

# Creamos un subconjunto solo con NAs con frecuencia paquetes
df_paquetes_na <- df_final %>%
  filter(is.na(frecuencia_paquetes))

# Se revisa el nivel educativo 
...
#Recodificando la variable frecuencia de consumo de paquetes 

df_analisis <- df_analisis %>%
  mutate(
    freq_paquetes_num = case_when(
      is.na(frecuencia_paquetes) ~ 7, 
      TRUE ~ as.numeric(frecuencia_paquetes) 
    ),
    
    # Variable Agrupada 
    consumo_paquetes_agrupado_txt = case_when(
      freq_paquetes_num == 7 ~ "No Responde/Missing",
      freq_paquetes_num == 6 ~ "Ocasional (< 1/sem)", 
      freq_paquetes_num %in% c(4, 5) ~ "Semanal (1-3/sem)", 
      freq_paquetes_num == 3 ~ "Frecuente (4-6/sem)", 
      freq_paquetes_num %in% c(1, 2) ~ "Diario (Todos los días)" 
    ),
    
    # Consumo Agrupado (Factor Ordenado)
    consumo_paquetes_agrupado = factor(consumo_paquetes_agrupado_txt, 
                                       levels = c("Ocasional (< 1/sem)", 
                                                  "Semanal (1-3/sem)",
                                                  "Frecuente (4-6/sem)",
                                                  "Diario (Todos los días)",
                                                  "No Responde/Missing")), 
    
    # Consumo Detallado (Factor Ordenado)
    consumo_paquetes_detallado = factor(freq_paquetes_num,
                                        levels = c(6, 5, 4, 3, 2, 1, 7), 
                                        labels = c("Menos de 1 vez/sem",
                                                   "1 vez/sem",
                                                   "2-3 veces/sem",
                                                   "4-6 veces/sem",
                                                   "Diario (1 vez)",
...
#### 2.2.6) Estado de salud general
```{r}
# Cálculo NO Ponderado (Muestra real)
tabla_no_pond_es <- df_analisis %>%
  count(estado_salud) %>%
  mutate(prop_no_pond = n / sum(n)) %>%
  rename(freq_no_pond = n)

# Cálculo Ponderado (Estimación poblacional)
tabla_pond_es <- df_analisis %>%
...
# Unir tablas
tabla_final_es <- tabla_no_pond_es %>%
  left_join(tabla_pond_es, by = "estado_salud")

print(head(tabla_final_es))

# Preparar datos para gráfico
datos_grafico_es <- tabla_final_es %>%
  select(estado_salud, prop_no_pond, prop_pond) %>%
  pivot_longer(
    cols = c(prop_no_pond, prop_pond),
    names_to = "tipo_medida",
    values_to = "proporcion"
...
# Gráfico comparativo
univariado_estado_salud <- ggplot(datos_grafico_es,
                                  aes(x = estado_salud,
                                      y = proporcion,
                                      fill = tipo_medida)) +
  geom_col(position = "dodge", width = 0.7) +
  ...
  labs(
    title = "Comparación: Muestra vs. Población Estimada",
    subtitle = "Distribución del estado de salud autopercibido",
    x = "Estado de salud",
    y = "Porcentaje (%)",
    fill = "Tipo de Medida",
    caption = "Fuente: Elaboración propia"
...
print(univariado_estado_salud)

# Guardar gráfico
ggsave("Graficos-tablas/univariado_estado_salud.png",
       univariado_estado_salud,
       width = 8,
       height = 6)

```

#### 2.2.7) Frecuencia de consumo alimentos de paquete
```{r}
# Cálculo NO Ponderado (Muestra real)
tabla_no_pond_fp <- df_analisis %>%
  count(frecuencia_paquetes) %>%
  mutate(prop_no_pond = n / sum(n)) %>%
  rename(freq_no_pond = n)

# Cálculo Ponderado (Estimación poblacional)
tabla_pond_fp <- df_analisis %>%
  group_by(frecuencia_paquetes) %>%
  summarise(freq_pond = sum(fex, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(prop_pond = freq_pond / sum(freq_pond))

# Unir tablas
tabla_final_fp <- tabla_no_pond_fp %>%
  left_join(tabla_pond_fp, by = "frecuencia_paquetes")

print(head(tabla_final_fp))

# Preparar datos para gráfico
datos_grafico_fp <- tabla_final_fp %>%
  select(frecuencia_paquetes, prop_no_pond, prop_pond) %>%
  pivot_longer(
    cols = c(prop_no_pond, prop_pond),
    names_to = "tipo_medida",
    values_to = "proporcion"
...
# Paleta de colores TEAL
paleta_teal <- c("#E0F7FA", "#80DEEA", "#26C6DA", "#0097A7", "#006064")

# Gráfico comparativo
univariado_paquetes <- ggplot(datos_grafico_fp,
                              aes(x = frecuencia_paquetes,
                                  y = proporcion,
                                  fill = tipo_medida)) +
  geom_col(position = "dodge", width = 0.7) +
  ...
  labs(
    title = "Comparación: Muestra vs. Población Estimada",
    subtitle = "Distribución de frecuencia de paquetes",
    x = "Categoría de frecuencia",
    y = "Porcentaje (%)",
    fill = "Tipo de Medida",
    caption = "Fuente: Elaboración propia"
  ) +
...
print(univariado_paquetes)

# Guardar gráfico
ggsave("Graficos-tablas/univariado_frecuencia_paquetes.png",
       univariado_paquetes,
       width = 8,
       height = 6)
```
...
# D) Por Estado de Salud (Validación de coherencia interna)
df_grafico_salud <- df_bivariado %>%
  group_by(estado_salud) %>%
  summarise(
    poblacion_total = sum(fex, na.rm = TRUE),
    poblacion_enferma = sum(fex[cronica_12m == "Sí"], na.rm = TRUE),
...
# E) Por Consumo de Paquetes (Factor de riesgo concurrente)
df_grafico_paquetes <- df_bivariado %>%
  # Excluimos la categoría de missing para el gráfico bivariado
  filter(consumo_paquetes_agrupado != "No Responde/Missing") %>% 
  group_by(consumo_paquetes_agrupado) %>%
  summarise(
    poblacion_total = sum(fex, na.rm = TRUE),
    poblacion_enferma = sum(fex[cronica_12m == "Sí"], na.rm = TRUE),
    prevalencia = poblacion_enferma / poblacion_total,
...
exportar_html(df_grafico_global, "Prevalencia Global Ponderada", "Bivariado_prevalencia_global_TABLA")
exportar_html(df_grafico_estratificado, "Prevalencia por Edad", "Bivariado_prevalencia_edad_TABLA")
exportar_html(df_grafico_sexo, "Prevalencia por Sexo y Edad", "Bivariado_prevalencia_edad_sexo_TABLA")
exportar_html(df_grafico_salud, "Prevalencia por Estado de Salud", "Bivariado_prevalencia_salud_TABLA")
exportar_html(df_grafico_paquetes, "Prevalencia por Consumo de Paquetes", "Bivariado_prevalencia_paquetes_TABLA")
```
...
# --- GRÁFICO 5: TENDENCIAS POR ESTADO DE SALUD ---
p_salud <- ggplot(df_grafico_salud, aes(x = estado_salud, y = prevalencia, fill = estado_salud)) +
  geom_col(width = 0.7, alpha = 0.9) +
  geom_text(aes(label = paste0(round(preva_100, 1), "%")), 
            vjust = -0.5, size = 4, fontface = "bold", color = "#2c3e50") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1), limits = c(0, 0.80)) + # Ajuste límite
  scale_fill_manual(values = c("#26C6DA", "#0097A7", "#006064", "#2c3e50")) + # Gradiente Teal a Oscuro
  labs(
    title = "Gradiente de enfermedad crónica según autopercepción de salud",
    subtitle = "Validación de constructo: a peor estado percibido, mayor prevalencia objetiva",
    x = "Estado de salud percibido", 
    y = "Prevalencia de enfermedad crónica (%)",
    caption = "Fuente: ECV 2023. Población expandida."
  ) +
  theme_minimal() +
  theme(
...
# --- GRÁFICO 6: TENDENCIAS POR CONSUMO DE PAQUETES ---
p_paquetes <- ggplot(df_grafico_paquetes, aes(x = consumo_paquetes_agrupado, y = prevalencia)) +
  geom_segment(aes(x = consumo_paquetes_agrupado, xend = consumo_paquetes_agrupado, 
                   y = 0, yend = prevalencia), color = "grey70", size = 1) +
  geom_point(size = 6, color = "#FF7F0E") + # Usamos naranja para contrastar con bebidas (Teal)
  geom_text(aes(label = paste0(round(preva_100, 1), "%")), 
            vjust = -1.8, size = 3.5, fontface = "bold", color = "#2c3e50") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1), limits = c(0, 0.25)) +
  labs(
    title = "Prevalencia de enf. crónica según consumo de paquetes",
    x = NULL, 
    y = "Prevalencia (%)"
  ) +
  theme_few() +
...
ggsave("Graficos-tablas/bivaariado_salud_enfermedad.png", p_salud, width = 12, height = 6)
ggsave("Graficos-tablas/bivaariado_paquetes_enfermedad.png", p_paquetes, width = 12, height = 6)
...
En cuanto al analisis bivariado de percepción de salud y prevalencia de enfermedad crónica se puede observar que a mayor autopercepción de mala salud, aumenta la prevalencia de enfermedad crónica. Este hallazgo valida la idea de que la coherencia y consistencia de la variable que estamos usando como dependiente. Por otro lado, también se puede observar una relación de causalidad reversa en la prevalencia de enfermedad crónica vs consumo de paquetes, este hallazgo refuerza la hipótesis de que el diagnóstico médico cambia el comportamiento de las personas enfermas y no permite ver claramente el efecto real del consumo de este tipo de alimentos sobre la salud en este tipo de encuestas.  
...
# PREPARAR DATAFRAME BASE PARA EL MODELO
