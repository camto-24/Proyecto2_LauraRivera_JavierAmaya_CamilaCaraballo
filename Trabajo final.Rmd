...
# Paleta personalizada (ya la tenías)
pal <- paletteer::paletteer_c("ggthemes::Blue-Teal", 30)

# =========================================
# 1. Preparación de la variable de 4 categorías
# =========================================
df_grafico_4cat <- df_analisis %>%
  ...
  mutate(
    ...
    freq_num_tmp = case_when(
      consume_azucar == 2 ~ 0,
      TRUE ~ as.numeric(as.character(frecuencia_azucar))
    ),
    ...
    # Re-codificar cronica_binaria a 0/1
    cronica_binaria_num = case_when(
      cronica_binaria %in% c("Sí", "si", "SI", 1) ~ 1,
      cronica_binaria %in% c("No", "no", "NO", 0) ~ 0,
      TRUE ~ NA_real_
    )
  ) %>%
  # Crear la variable de grupo edad
  mutate(
...
# =========================================
# 2. Cálculo de Estadísticas Ponderadas
# =========================================
summary_4cat_ponderado <- df_grafico_4cat %>%
  ...
  summarise(
    ...
    prevalencia = weighted.mean(cronica_binaria_num, w = fex, na.rm = TRUE),
    
    # Error estándar aproximado
    se = sqrt((prevalencia * (1 - prevalencia)) / n_muestral),
    
    # Intervalo de Confianza 95%
    lim_inf = pmax(prevalencia - 1.96 * se, 0),
    lim_sup = pmin(prevalencia + 1.96 * se, 1),
...
# 3. Visualización
# =========================================
grafico2 <- ggplot(summary_4cat_ponderado, aes(x = nivel_consumo, y = prevalencia, fill = nivel_consumo)) +
  geom_col(alpha = 0.8) +
  geom_errorbar(aes(ymin = lim_inf, ymax = lim_sup), width = 0.2, color = "black", linewidth = 0.7) +
  facet_wrap(~grupo_edad, scales = "free_y") +
  labs(
    title = "Prevalencia por Nivel de Consumo (4 Categorías)",
    subtitle = "Estimación ponderada (FEX) con intervalos de confianza del 95%",
  ...
  scale_fill_manual(values = pal[c(5, 10, 20, 30)]) +
  theme_minimal() +
  theme(
...
# 4. Guardar gráfico
# =========================================
ggsave("Graficos-tablas/grafico2.png", plot = grafico2, width = 10, height = 6, dpi = 300)
...
summary_con_ic <- df_grafico_ponderado %>%
  group_by(grupo_edad, frecuencia_full) %>%
  summarise(
    n_muestral = n(),
    prevalencia = weighted.mean(cronica_binaria, w = fex, na.rm = TRUE),
...
# 3. Gráfica
grafico3 <- ggplot(summary_con_ic, aes(x = frecuencia_full, y = prevalencia, fill = frecuencia_full)) +
  geom_col(alpha = 0.8) + 
  geom_errorbar(aes(ymin = lim_inf, ymax = lim_sup), width = 0.2, color = "black", linewidth = 0.7) +
  facet_wrap(~grupo_edad, scales = "free_y") +
  labs(
...
ggsave("Graficos-tablas/prevalencia_poblacional.png", 
       plot = grafico3, 
       width = 10, 
       height = 6, 
       dpi = 300)
...
# Guardar
tabla_cruce_gt <- tabla_cruce_fex %>%
  gt() %>%
  tab_header(
    title = "Distribución de Enfermedad Crónica por Consumo",
    subtitle = "Análisis Ponderado (FEX)"
  ) %>%
  # Opcional: Añadir estilo básico
  opt_row_striping() 

# Guardamos el archivo HTML
gtsave(tabla_cruce_gt, filename = "Graficos-tablas/tabla_cruce_larga.html")

# 2. Visualizar la tabla en formato "Largo" 
print(tabla_cruce_fex)


# OPCIÓN B: Formato "Ancho" 
...
# Guardar
tabla_matriz_gt <- tabla_matriz %>%
  gt() %>%
  tab_header(
    title = "Resumen de Prevalencia y Población",
    subtitle = "Tabla Matriz Ponderada"
  ) 

# Guardamos el archivo HTML
gtsave(tabla_matriz_gt, filename = "Graficos-tablas/tabla_matriz_resumen.html")
```
...
# Guardar tabla
tabla_modelo <- resultados_or %>%
  gt() %>%
  # Títulos
  tab_header(
    title = "Resultados del Modelo de Regresión Logística",
    subtitle = "Odds Ratios (OR) ajustados por edad, sexo e ingreso"
  ) %>%
  # Renombrar columnas
  cols_label(
    term = "Variable",
  ...
  fmt_number(
    columns = c(estimate, std.error, conf.low, conf.high, p.value),
    decimals = 3
  ) %>%
  # Estilo
  tab_options(
...
# 2. Guardar en la carpeta
gtsave(tabla_modelo, filename = "Graficos-tablas/resultados_modelo.html")
```
