...
La base fue cargada correctamente y contiene 240.212 observaciones y 14 variables, tal como se observa en el resumen general generado por skim(). La estructura también se verificó: el conjunto tiene 12 variables numéricas y 2 variables de tipo carácter.
...
tabla_resumen <- df_final %>%
  summarise(across(
    everything(),
    list(
      tipo = ~ as.character(class(.x)[1]),
      na = ~ as.character(sum(is.na(.x))),
      na_porc = ~ as.character(round(mean(is.na(.x)) * 100, 2)),
      min = ~ if (is.numeric(.x)) as.character(min(.x, na.rm = TRUE)) else NA,
      max = ~ if (is.numeric(.x)) as.character(max(.x, na.rm = TRUE)) else NA,
      media = ~ if (is.numeric(.x)) as.character(round(mean(.x, na.rm = TRUE), 2)) else NA,
      mediana = ~ if (is.numeric(.x)) as.character(median(.x, na.rm = TRUE)) else NA,
      n_categorias = ~ if (!is.numeric(.x)) as.character(length(unique(.x))) else NA
    ),
    .names = "{col}__{fn}"
  )) %>%
  pivot_longer(
...
save_kable(tabla_html, "Graficos-tablas/tabla_resumen_df_final.html")
